...
Summarise DBD- TF isoforms: 

1) dominant in at least one tissue and expressed above 1 in at least one tissue of dominance; 

2) don't become dominant or become dominant below the noise level; expressed above 1 in at least one tissue; 

(1) and (2) can be sequestering factors or cofactors.

3) expressed below 1 in all tissues (leaky expression).

```{r, include=T}
# all.dbd.minus.n = length(c(dbd.negative$ensembl_transcript_id, domainless.isoforms$ensembl_transcript_id))
#   
# dbd.minus.expressed.n = 
#   
# dbd.minus.dominant.expressed.n = 
#   
# data.frame(Group = c("All DBD-", "Expressed in >=1 tissue", "Expressed in >=1 tissue as dominant"),
#            Category = rep("DBD-", 3),
#            Fraction = c()) %>%
...
```{r, include=T}
make_line_plot("CREM", T)
make_line_plot("CREM", F)
make_domain_table("CREM", T)
make_domain_table("CREM", F)
gene.id = tf.family.table %>%
  filter(humantfs_gene_name == "CREM") %>%
  pull(ensembl_gene_id)
ensg_enst_tsl %>%
  filter(ensembl_gene_id == gene.id)
```
...
# isoform.expression.tf.annot =  #base.table %>%
#   left_join(isoform.expression.all %>%
#               tibble::rownames_to_column(),
#             by = c("ensembl_transcript_id" = "rowname")) %>%
#   arrange(ensembl_gene_id, ensembl_transcript_id)
